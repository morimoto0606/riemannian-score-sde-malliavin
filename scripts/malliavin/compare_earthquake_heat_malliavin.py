...
DEFAULT_HEAT_RUN_DIR = (
    REPOSITORY_ROOT / "results/earthquake_upstream_heat_baseline"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--heat-run-dir",
        type=Path,
        default=DEFAULT_HEAT_RUN_DIR,
    )

    parser.add_argument(
        "--malliavin-run-dir",
        type=Path,
        default=DEFAULT_MALLIAVIN_RUN_DIR,
    )

    parser.add_argument(
        "--spectrum-run-dir",
        type=Path,
        help="Spectrum run directory",
    )
...
def resolve_samples_path(
    ...
    *,
    ...
    candidates.append(
        ("run directory", run_dir / "generated_samples.npy")
    )
    ...
    if saved_setting is not None:
        candidates.append(
            ("saved Hydra config", _localise_saved_path(saved_setting))
        )
    ...
    for source, path in candidates:
        ...
        if resolved.is_file():
            print(
                f"{label} samples: {resolved} ({source})"
            )
            return resolved
    ...
    raise FileNotFoundError(
        ...
        + "\n".join(checked)
        + f"\nThe checkpoint, if present, is {run_dir / 'ckpt'}."
    )
...
def load_generated(
    ...
    points = validate_s2_points(
        raw,
        f"{label} generated samples",
    )
...
def save_scatter_comparison(
    ...
    *,
    ...
    for column, (title, latlon, color) in enumerate(
        datasets,
        start=1,
    ):
...
def save_density_comparison(
    ...
    *,
    ...
    if (
        not np.isfinite(common_maximum)
        or common_maximum <= common_minimum
    ):
        raise FloatingPointError(
            "shared density grid is invalid or identically zero"
        )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    malliavin_run_dir = (
        args.malliavin_run_dir.expanduser().resolve()
    )
    ...
    heat_samples_path = resolve_samples_path(
        heat_run_dir,
        args.heat_samples_path,
        label="Heat",
    )

    malliavin_samples_path = resolve_samples_path(
        malliavin_run_dir,
        args.malliavin_samples_path,
        label="Malliavin",
    )


    real_latlon = load_earthquake_latlon(
        args.data_path.expanduser().resolve()
    )

    real_points = latlon_to_upstream_s2(
        real_latlon
    )
    ...
    print(
        f"map centre: latitude={central_lat:.3f}, "
        f"longitude={central_lon:.3f}"
    )
