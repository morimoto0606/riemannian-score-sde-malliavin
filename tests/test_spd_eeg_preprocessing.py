...
class EEGTests(unittest.TestCase):
    def test_subject_disjoint_and_spd(self):
        x=np.random.RandomState(4).normal(size=(40,15,30))
        ...
        self.assertEqual(d['covariances'].shape, (40,15,15))
        self.assertEqual(m['dimension'],15)
    ...
    def test_nonfinite_rejected(self):
        with self.assertRaises(ValueError):
            b.prepare(np.full((10,15,30),np.nan),np.tile([0,1],5),np.arange(10))

    def test_wrong_channel_count_explains_shape(self):
        with self.assertRaisesRegex(ValueError, '15 channels.*got'):
            b.prepare(np.ones((10,6,30)),np.tile([0,1],5),np.arange(10))
