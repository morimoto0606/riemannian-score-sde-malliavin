...
Prepare 36 runs: earthquake and volcano 600000 updates, flood 300000 updates;
Malliavin, pure spectrum, pure Varadhan and ISM; seeds 0,1,2.
...
Malliavin uses one probe, no RB, and an explicitly selected lambda (0 or 5).
...
Example preparation (choose lambda deliberately):

```bash
python scripts/prepare_earth_long_comparison.py --output "$PWD/results/earth_long_v1" --malliavin-lambda 5 --check-only
python scripts/prepare_earth_long_comparison.py --output "$PWD/results/earth_long_v1" --malliavin-lambda 5
...
and repeated subjob starts are refused. The entire 36-job array need not execute
